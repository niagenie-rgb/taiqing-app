...
// ========== 共用：兼容新舊格式抓繳費資料 ==========
// 舊資料只有 year/month，新資料有 payYear/payMonth
// 同時查兩個欄位並去重，確保不漏筆
...
async function loadPaySummary(year, month) {
  ...
  // 加入 doc id 用於刪除（需另外查）
  const snap = await getDocs(query(collection(db, 'payments'), where('payYear', '==', year), where('payMonth', '==', month)));
  const snap2 = await getDocs(query(collection(db, 'payments'), where('year', '==', year), where('month', '==', month)));
  const listMap = new Map();
  snap.docs.forEach(d => listMap.set(d.id, { id: d.id, ...d.data() }));
  snap2.docs.forEach(d => listMap.set(d.id, { id: d.id, ...d.data() }));
  const list = Array.from(listMap.values()).filter(p => (p.payMonth ?? p.month) === month);
...
function buildReportHTML(year, month, prev, units, displayMap, thisMonthPays, fins, mgmt, otherInc, exp, totalInc, net, bal) {
  ...
  return '<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8">'
    ...
    + 'body{font-family:"Microsoft JhengHei","PingFang TC",Arial,sans-serif;font-size:11pt;color:#111;padding:0}'
    ...
    + 'td,th{border:1px solid #888;padding:4px 4px;font-size:10pt;overflow:hidden;white-space:nowrap}'
    ...
    + '</table>'
    + '<table class="sum-bar" style="margin-top:2px"><colgroup><col style="width:12%"><col style="width:9%"><col style="width:12%"><col style="width:9%"><col style="width:58%"></colgroup>'
    + '<tr>'
...
async function loadSummaryTable(year) {
  ...
  const pays = await fetchPaymentsByYear(year);
