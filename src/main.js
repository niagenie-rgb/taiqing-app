...
function buildReportHTML(year, month, prev, units, displayMap, thisMonthPays, fins, mgmt, otherInc, exp, totalInc, net, bal) {
  ...
  return '<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8">'
    ...
    + 'body{font-family:"Microsoft JhengHei","PingFang TC",Arial,sans-serif;font-size:10pt;color:#111;padding:0}'
    + 'h1{text-align:center;font-size:13pt;font-weight:bold;padding:5px 0 7px;border-bottom:2px solid #333;margin-bottom:6px}'
    + '.top-bar{display:flex;justify-content:space-between;align-items:center;margin-bottom:4px}'
    + '.sec{font-weight:bold;font-size:11pt;color:#1a3a7a;border-left:4px solid #1a3a7a;padding-left:5px}'
    + '.prev-bal{font-size:10.5pt;font-weight:bold}'
    + 'table{width:100%;border-collapse:collapse}'
    + 'td,th{border:1px solid #888;padding:3px 3px;font-size:9pt;overflow:hidden;white-space:nowrap}'
    + '.ut th{background:#1a3a7a;color:white;font-weight:bold;font-size:8.5pt;text-align:center;padding:3.5px}'
    + '.ut td{font-size:8.5pt;height:18px}'
    + '.sum-bar{background:#e8eef8;font-weight:bold}'
    + '.sum-bar td{padding:4px 5px;font-size:9.5pt;border:1px solid #888}'
    + '.et th{background:#2a5a2a;color:white;font-weight:bold;font-size:9.5pt;text-align:center;padding:4px}'
    + '.et td{font-size:9.5pt;padding:3px 4px;height:20px}'
    + '.tot-row td{background:#1a3a7a;color:white;font-weight:bold;font-size:11pt;padding:5px 6px;border:1px solid #0d2050}'
    + '.r{text-align:right}'
    + '.note{font-size:8.5pt;color:#555;margin-top:5px;padding:3px 6px;border:1px solid #aaa;background:#f8f8f8}'
    + '.sign{display:flex;gap:20px;margin-top:10px}'
    + '.sign div{flex:1;border-bottom:1px solid #333;padding-bottom:4px;font-size:10.5pt}'
    + '</style></head><body>'
    ...
    + '<table class="sum-bar" style="margin-top:3px"><colgroup><col style="width:100%"></colgroup>'
    + '<tr><td><strong>其他收入原因：</strong>' + (otherDetail||'無') + '</td></tr>'
    + '</table>'
    + '<table class="sum-bar" style="margin-top:2px"><colgroup><col style="width:28%"><col style="width:17%"><col style="width:28%"><col style="width:14%"><col style="width:13%"></colgroup>'
    + '<tr>'
    + '<td><strong>B. 管理費收入：</strong></td><td class="r"><strong>' + mgmt.toLocaleString() + ' 元</strong></td>'
    + '<td><strong>C. 其他收入：</strong></td><td class="r"><strong>' + otherInc.toLocaleString() + ' 元</strong></td>'
    + '<td class="r" style="background:#1a3a7a;color:white;text-align:center"><strong>D.合計<br>' + totalInc.toLocaleString() + '</strong></td>'
    + '</tr></table>'
